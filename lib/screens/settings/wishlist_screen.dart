...
class _WishlistScreenState extends State<WishlistScreen> {
  ...
  void _loadWishlist() {
    setState(() {
      _wishlistIds = StorageService.getWishlist();
    });
  }
              ...
              child: ListView(
                children: [
                  _buildCartItem('Tenda Eiger 4P', 'Rp 50.000',
                      'https://picsum.photos/seed/quyg4d/600/400'),
                  _buildCartItem('Matras Ultralight', 'Rp 15.000',
                      'https://picsum.photos/seed/l2h96q/600/400'),
                ],
              ),
            ),
            Container(
          ...
          if (_wishlistIds.isEmpty)
            Center(
              child: Column(
                children: [
                  const SizedBox(height: 40),
                  Icon(Icons.heart_broken, size: 80, color: Colors.grey.shade300),
                  const SizedBox(height: 16),
                  const Text('Belum ada perlengkapan di Wishlist', style: TextStyle(color: Colors.grey, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 60),
                ],
              ),
            )
          else
            ..._wishlistIds.map((id) => _buildWishlistItem(
                id, // Use ID as placeholder name
                '4.8',
                '128',
                '50.000',
                'https://picsum.photos/seed/8bohvd/600/400',
                primaryColor,
                id,
            )),
// END REPLACE
  ...
  Widget _buildHeaderStats(Color primaryColor) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(_wishlistIds.length.toString().padLeft(2, '0'),
            style: TextStyle(
                fontFamily: 'Manrope',
                fontSize: 56,
                fontWeight: FontWeight.w800,
  ...
  Widget _buildWishlistItem(String title, String rating, String reviews,
      String price, String imageUrl, Color primaryColor, [String id = '']) {
    return Container(
                ...
                Row(
                    ...
                    children: [
                      ...
                      IconButton(
                        ...
                        onPressed: () async {
                          if (id.isNotEmpty) {
                            await StorageService.toggleWishlist(id);
                            _loadWishlist();
                          }
                        },
                      ),
